...
az appservice plan create --name $appsrv1 --resource-group $rg --location $location1 --sku Free # B1  --is-linux 
az webapp create --resource-group $rg --plan $appsrv1 --name $webapp1 --deployment-local-git --% --runtime "PYTHON|3.4"  
...
#az webapp create --resource-group $rg --plan $appsrv2 --name $webapp2 --deployment-local-git --startup-file "api:app" --% --runtime "PYTHON|3.7" 
